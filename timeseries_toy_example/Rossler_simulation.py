...
class SDE(torch.nn.Module):
    ...
    def f(self, t, y):
        ...
        if t == 0:
            ...
            outs = torch.sqrt(1 - (self.sigma ** 2) * t * (1 - t))(predicts[:, :DataDim] - y) / .01 - (
                        predicts[:, DataDim:] / 2) * self.sigma * torch.sqrt((t) * (1 - t))
        else:
...
inputs=np.concatenate([X[:,None],Y[:,None],Z[:,None]],axis=-1)
scaler = MinMaxScaler(feature_range=(-1, 1))
ys = 10*scaler.fit_transform(inputs.reshape(-1, 1)).reshape(inputs.shape)
dataset=torch.tensor(ys).float().to(device)
...
if training_enable:
    for k in range(40000):
        ...
        if (k+1 ) % 2000 == 0:
            ...
            fig, ax = plt.subplots(1, 2,sharey=True,sharex=True, subplot_kw={'projection': '3d'})
    ...
    for kk in range(trjs_s):
        ax[0].plot(y_hat[1:, kk, 0], y_hat[1:, kk, 1], y_hat[1:, kk, 2], color='r')
    ax[1].plot(dataset.cpu().numpy()[:, 0],
               dataset.cpu().numpy()[:, 1],
               dataset.cpu().numpy()[:, 2],
               color='g', alpha=0.5)
    ...
    ax[0].set_title('Rossler Attractor')
