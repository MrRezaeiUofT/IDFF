...
flags.DEFINE_integer("img_dim", 64, help="image size")
# Training
...
flags.DEFINE_integer("batch_size", 200, help="")
...
new_net = UNetModelWrapper(
        dim=(6, FLAGS.img_dim, FLAGS.img_dim),
        num_res_blocks=2,
...
# Load the model
PATH = f"{FLAGS.input_dir}/{FLAGS.model+'-'+str(FLAGS.flow_w)+'-'+ str(FLAGS.sigma)}/{FLAGS.model}_celeba_weights_step_final.pt"
print("path: ", PATH)
...
def gen_1_img():
    with torch.no_grad():
        ...
        if FLAGS.integration_method == "euler":
            ...
            print("Use method: ", FLAGS.integration_method)
            sde = SDE(new_net, reverse=False, model_name=FLAGS.model, sigma=2*FLAGS.sigma)
            with torch.no_grad():
